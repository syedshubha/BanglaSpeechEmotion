function [t_labels1,predicted_labels1] = KnnClassifier2(k,data,labels,t_data,t_labels)
...
fprintf('Preparing k-NN classifier..\n');
%initialization
...
for i=1:4
t_labels1(i,:)=t_labels(length(t_labels)/4*(i-1)+1:length(t_labels)/4*i);

predicted_labels1(i,:)=predicted_labels(length(t_labels)/4*(i-1)+1:length(t_labels)/4*i);
end

% ConfMat = confusionmat(t_labels, predicted_labels);
% % ConfMat = plotconfusion(t_labels, predicted_labels);
